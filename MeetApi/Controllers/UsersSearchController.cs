...
public class UsersSearchController : Controller
{
    ...
    public UsersSearchController(UserManager<User> userManager, AppContext db)
    {
    ...
    private Task<List<UserForSearch>> GetUsers(User mainUser)
    {
        var coincidenceToUser = new Dictionary<UserForSearch, int>();
        foreach (var user in _db.Users)
        {
            ...
            coincidenceToUser.Add(new UserForSearch(user), count);
        }
    ...
    public Task<IActionResult> GetUsersByFilter(Filter filter)
    {
        ...
        var users = new List<UserForSearch>();

        foreach (var user in result)
        {
            users.Add(new UserForSearch(user));
        }
